...
export default function AdminModal({ 
    ...
    useEffect(() => {
        if (!active || activeTab !== 'palette') return;
        ...
        iroColorPicker.current = new iro.ColorPicker(colorPickerRef.current, {
            ...
            margin: 16,
            sliderHeight: 16,
            layoutDirection: 'vertical', // Sliders under the wheel to prevent wrapping bugs
            layout: [
    ...
    }, [active, activeTab]); // Do not depend on baseColor here to prevent recreation on every drag

    // Sync external baseColor changes to the picker
    useEffect(() => {
        if (iroColorPicker.current && baseColor && iroColorPicker.current.color.hexString.toLowerCase() !== baseColor.toLowerCase()) {
            iroColorPicker.current.color.hexString = baseColor;
        }
    }, [baseColor]);
    ...
    const renderContent = () => {
        switch(activeTab) {
            ...
            case 'uptime':
                return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        <div className="form-group">
                            <label>Uptime Kuma API 地址</label>
                            <input 
                                type="text" 
                                value={kumaUrl || ''} 
                        ...
                        <div className="form-group">
                            <label>状态页 Slug (Status Page Slug)</label>
                            <input 
                                type="text" 
                                value={kumaSlug || ''} 
            ...
            default:
                return (
                    <div className="palette-settings-tab">
                        <div className="palette-settings-column">
                            <div className="form-group">
                            ...
                            <div className="form-group">
                                ...
                                {hasMedia && (
                                    <button className="btn-secondary" onClick={onResetMedia} style={{ width: '100%', marginTop: '0.8rem' }}>恢复默认光斑背景</button>
                                )}
                            </div>
                        </div>
    ...
    return (
        <div className={`admin-overlay ${active ? 'active' : ''}`}>
            <div className={`admin-panel-wrapper ${activeTab === 'social' || activeTab === 'palette' ? 'wide-panel' : ''}`}>
